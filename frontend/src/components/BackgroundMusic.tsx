...
import { toggleMusic, setMusicVolume, toggleSoundEffects, setSoundEffectsVolume } from '../store/slices/audioSlice';

const BackgroundMusic = () => {
  const audioRef = useRef<HTMLAudioElement | null>(null);
  const dispatch = useAppDispatch();
  const { musicEnabled, musicVolume, soundEffectsEnabled, soundEffectsVolume } = useAppSelector((state) => state.audio);
  ...
  const handleSoundEffectsVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newVolume = parseFloat(e.target.value);
    dispatch(setSoundEffectsVolume(newVolume));
  };

  // Calculate expanded control states
  const [showSFXControls, setShowSFXControls] = useState(false);

  return (
    <div className="fixed bottom-4 right-4 z-50">
      <div className="bg-black/40 backdrop-blur-sm p-2 rounded-full flex items-center space-x-2">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            onMouseEnter={() => setShowSFXControls(true)}
          ...
          {(showSFXControls || soundEffectsEnabled) && (
            <div 
              className="px-2 flex items-center space-x-2"
              onMouseLeave={() => setShowSFXControls(false)}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white">
                <path d="M2 10L2 14C2 16 3 17 5 17H7" />
                <path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path>
              </svg>
              <input 
                type="range" 
